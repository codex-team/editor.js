...
export default class RectangleSelection extends Module {
  ...
  public prepare(): void {
    ...
    Listeners.on(document.body, 'mouseleave', this.endSelection, false);
  ...
  private prepareUI() {
      ...
      const rectContainer = $.make('div', RectangleSelection.CSS.overlayContainer, {});
      const rectangle = $.make('div', RectangleSelection.CSS.rect, {});

      overlay.appendChild(rectContainer);
      rectContainer.appendChild(rectangle);
      this.Editor.UI.nodes.wrapper.appendChild(overlay);

      this.rectangle = rectangle as HTMLDivElement;
  }
  ...
  private startSelection = (event: MouseEvent): void => {
    ...
    this.startX = this.mouseX = event.pageX - window.pageXOffset;
    this.startY = this.mouseY = event.pageY - window.pageYOffset;

    this.rectangle.style.left = `${this.startX}px`;
    this.rectangle.style.top = `${this.startY}px`;
  }

  private changeRectangle = (event: MouseEvent): void => {
    if (!this.mousedown) {
      return;
    }
    ...
    const width = Math.abs(this.mouseX - this.startX);
    const height = Math.abs(this.mouseY - this.startY);

    if (this.mouseX < this.startX) {
      this.rectangle.style.left = `${this.mouseX}px`;
    }

    if (this.mouseY < this.startY) {
      this.rectangle.style.top = `${this.mouseY}px`;
    }
  ...
  private endSelection = (): void => {
    ...
    this.startX = this.mouseX = 0;
    this.startY = this.mouseY = 0;

    this.rectangle.style.width = this.rectangle.style.height = '0';
  }
